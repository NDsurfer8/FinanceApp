...
export const BudgetScreen: React.FC<BudgetScreenProps> = ({ navigation }) => {
  ...
  const hasOverBudgetItems = () => {
    ...
    return budgetCategories.some((category) => {
      ...
      // Include all recurring expenses for this month (regardless of status)
  ...
  const getInsights = () => {
    ...
    const isOverBudget =
      totalOverBudget > 0 || (totalIncome > 0 && remainingBalance < 0);
    const overBudgetAmount =
      totalOverBudget > 0 ? totalOverBudget : Math.abs(remainingBalance);
